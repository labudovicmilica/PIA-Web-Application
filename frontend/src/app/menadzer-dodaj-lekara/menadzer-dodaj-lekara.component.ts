...
export class MenadzerDodajLekaraComponent implements OnInit {
  ...
  constructor(private userService: UserService, private router: Router, private http: HttpClient, private menadzerService: MenadzerService) { }
    ...
    if(!this.velicina && !this.ekstenzija && this.error == "" && this.choosen){
      ...
      await this.http.post<any>('http://localhost:3000/file', formData).subscribe((res)=>{
        if(res){
          ...
          this.menadzerService.addUser(this.ime, this.prezime, this.korisnickoIme, this.lozinka, this.mejl, this.adresa, this.telefon, "korisnik_default.png", this.spec, this.ogranak, this.licenca).subscribe((res)=>{
            if(res['message'] != 'ok'){
              alert("nonono")
            }
          })
          this.message = "Novi nalog lekara je dodat."
        }
      });
    }
